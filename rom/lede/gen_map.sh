...
echo get bins
bins="`find bin/targets/ | grep -- '\(-ext4-sdcard\|-squashfs\|-factory\|-sysupgrade\|-initramfs-factory\|-initramfs-recovery\|-preloader\|-bl31-uboot\)' | grep "natcap\|x-wrt" | grep -v vmlinux | grep -v '\.dtb$' | while read line; do basename $line; done`"
...
##################################
		...
		bin=`echo "$bins" | grep $arch | grep -i "\($name-ext4-sysupgrade\|$name-ext4-sdcard\|$name-squashfs\|$name-factory\|$name-initramfs-factory\|$name-initramfs-recovery\|$name-preloader\|$name-bl31-uboot\)"`
		test -n "$bin" || {
			name=`echo $name | tr _ -`
