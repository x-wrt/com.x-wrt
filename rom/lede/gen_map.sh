...
##################################
		...
		bin=`echo "$bins" | grep $arch | grep -i "\($name-ext4-sdcard\|$name-squashfs\|$name-factory\|$name-initramfs-factory\|$name-initramfs-recovery\|$name-preloader\|$name-bl31-uboot\)"`
		test -n "$bin" || {
			name=`echo $name | tr _ -`
